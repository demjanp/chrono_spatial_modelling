...
def calculate_PCF_solutions(solutions, coords, eu_side):
	...
	def calculate_PCF_phase(solutions, pi, coords, Sx, Sy, r_max, eu_side):
		...
		for i in range(num_increments):
			...
			if g_i.size:
				g_average[i] = np.mean(g_i) / (np.pi * (rOuter**2 - rInner**2))
			else:
				g_average[i] = np.nan
	...
	pcf = np.array(pcf) # pcf[pi] = [r, g]; where pi = index of phase, r = radius of the annulus used to compute g(r), g = average correlation function g(r)
...
def calculate_PCF_randomized(solutions, path_coords_examined, extent, eu_side, randomize_n):
	...
	coords_examined = coords_examined[(coords_examined[:,0] > extent[0]) & (coords_examined[:,0] < extent[1]) & (coords_examined[:,1] > extent[2]) & (coords_examined[:,1] < extent[3])]
	...
	for pi in range(phases_n):
		...
		for ri in range(randomize_n):
			pcf_rnd = calculate_PCF(coords_examined[np.random.choice(coords_examined.shape[0], coords_n[pi], replace = False)], r_max, eu_side) # [[r, g], ...]; where r = radius of the annulus used to compute g(r), g = average correlation function g(r) based on all solutions
