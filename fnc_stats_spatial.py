...
def get_cost(grid_slope, grid_water, water_limit):
	...
	for pnt in water_crds:
		for i in range(coords_x.shape[0]):
			...
			pnt2 = pnt + coords_x[i]
			if (pnt2[0] >= 0) and (pnt2[1] >= 0) and (pnt2[0] < grid_water_c.shape[0]) and (
					pnt2[1] < grid_water_c.shape[1]) and (grid_water_c[pnt2[0], pnt2[1]] >= water_limit):
				i *= 2
				if i == 0:
				...
				if (pnt3[0] >= 0) and (pnt3[1] >= 0) and (pnt3[0] < grid_water_c.shape[0]) and (
						pnt3[1] < grid_water_c.shape[1]) and (pnt4[0] >= 0) and (pnt4[1] >= 0) and (
						pnt4[0] < grid_water_c.shape[0]) and (pnt4[1] < grid_water_c.shape[1]) and (
						grid_water_c[pnt3[0], pnt3[1]] < water_limit) and (
						grid_water_c[pnt4[0], pnt4[1]] < water_limit):
...
def get_accu_cost(seeds, grid_cost, grid_vert, limits=None):
	...
	while seeds_collect:
		...
		mask = ((crds[:, 0] >= 0) & (crds[:, 1] >= 0) & (crds[:, 0] < grid_cost.shape[0]) & (
					crds[:, 1] < grid_cost.shape[1]))
		crds = crds[mask]
		if crds.shape[0]:
			dist = dist_o[mask]
...
def get_production_areas(coords, dem, cost_surface, vertical_component, production_area):
	...
	limit = int(
		round(production_area * (100 / dem.get_cell_size()[0]) ** 2))  # max. number of raster cells per production area
	production_areas = []
	cmax = len(coords)
	c = 1
	for x, y in coords:
...
def find_neighbours(coords, exclude, eu_side):
	...
	for i in range(coords.shape[0]):
		neighbours.append(
			np.where((np.abs(coords[i] - coords) <= [2.2 * eu_side, 1.1 * eu_side]).all(axis=1))[0].astype(
				int).tolist())
